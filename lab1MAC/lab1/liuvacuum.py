...
class LIUVacuumEnvironment(Environment):
    ...
    """
    def __init__(self, env_x=5, env_y=5, dirt_bias=0.1, dirt_distribution=0, wall_bias=0.0, world_seed=None):
        super().__init__()
        self.env_x = env_x
        self.env_y = env_y
    ...
    def execute_action(self, agent, action):
        ...
        self.update_performance(agent, action)
    ...
    def dirtify(self, randfunc):
        uniform = (self.dirt_distribution == 0)
            ...
            for y in range(self.env_y-1):
                if (self.world[x][y] != ENV_WALL):
                    if randfunc() < self.dirt_bias:
                        ...
                        if (not uniform):
                            if x > 1:
                                self.world[x-1][y] = ENV_DIRTY
                            if x < self.env_x - 2:
                                self.world[x+1][y] = ENV_DIRTY
                            if y > 1:
                                self.world[x][y-1] = ENV_DIRTY
                            if y < self.env_y - 2:
                                self.world[x][y+1] = ENV_DIRTY
