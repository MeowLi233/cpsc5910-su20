...
from venv.liuvacuum import LIUVacuumEnvironment, ENV_CLEAN, ENV_DIRTY, ENV_WALL, ENV_GOLD
...
class Lab1:
    ...
    def __init__(self):
        ...
        self.recon_getter = self.create_selection_menu(
                self.update_all,
                ("Full", "Full"), ("None", "None"), ("Summary", "Summary"),
                always_trigger_onselect=True
        )
    ...
    def create_sim(self):
        ...
        """
        venv = LIUVacuumEnvironment(*STANDARD_ENV_DIMS, 
                                    self.dirt_bias_getter(), 
                                    self.wall_bias_getter(),
                                    self.seed_getter())
        file_name, agent_name = self.agent_getter()   # this is (file, name)
        spec = importlib.util.spec_from_file_location(agent_name, file_name)
        agent_mod = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(agent_mod)
        aclass = eval(f"agent_mod.{agent_name}")
        agent = aclass(self.append_log)
        venv.add_thing(agent)
        self.agent = agent
        self.vacuum_env = venv
        recon = self.recon_getter()
        if recon == "Full" or recon == "Summary":
            venv.prep_agent(agent, recon)
